...
function widthInferenceCompleted(status::WidthInfereceStatus)
    status.wireWithUnresolvedWidthCount == 0
end
function widthInferenceMadeProgress(status::WidthInfereceStatus)
    (
...
function throwIfInferenceIsIncomplete(status::WidthInfereceStatus)
...
function autodeclCore(x, env::Vmodenv)
    ...
    finalItemCountWithUnknownWidth = countUnknownWidthItem.((env.prts, env.dcls))
    inferenceStatus = WidthInfereceStatus(
...
"""
    autodecl(x, env::Vmodenv)::Tuple{WidthInfereceStatus, Vmodenv}
...
```
"""
function autodecl(x, env::Vmodenv)::Tuple{WidthInfereceStatus, Vmodenv}
    status, (d, nenv) = autodeclCore(x, env)
    vpush!(nenv.dcls, d)
    return status, nenv
end
...
"""
    autodecl(x::Vmodule)::Tuple{WidthInfereceStatus, Vmodule}
...
"""
function autodecl(x::Vmodule)::Tuple{WidthInfereceStatus, Vmodule}
    env = Vmodenv(x)
    ...
    status, nenv = autodecl(VmodBody(x), env)
