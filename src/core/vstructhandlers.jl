...
julia> ipts = invports(pts);

julia> vshow(pts); vshow(ipts);
(
    input [7:0] bus1,
    input [7:0] bus2,
    output reg bus3
...
"""
function vfinalize(x::Vmodule; rst=defrst)
    m = autoreset(x; rst=rst)
    status, m = autodecl(m)
...
"""
    vfinalize(x::Vector{Vmodule})

Return new `Vmodule` objects generated by applying 
`autoreset` and `autodeclVmodlist` on `x`.
"""
function vfinalize(x::Vector{Vmodule}; rst=defrst)
    reset = autoreset.(x; rst=rst)
    status, mods = autodeclVmodlist(reset)
    throwIfInferenceIsIncomplete.(status)
    return mods
end
